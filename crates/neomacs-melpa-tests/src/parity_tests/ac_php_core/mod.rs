...
const AC_PHP_CORE_TEST_TIMEOUT: Duration = Duration::from_secs(180);

/// ac-php-core is the half of ac-php that does the work: it finds the project
/// root, writes and reads the per-project configuration, runs the bundled
/// `phpctags' PHAR to build an index, loads and merges that index, resolves
/// the type of an expression at point, and implements jump-to-definition and
/// the location stack.  ac-php itself is only the auto-complete front end.
///
/// There is no PHP interpreter on the test host, so `php' is the one stand-in,
/// and the index it replays is a recording rather than an invention: the exact
...
/// replaced by a token.  This is the same recording the `ac_php' suite uses,
/// for the same reason -- a hand-written index would pin a reconstruction of
/// phpctags' format rather than the package's behaviour.
///
/// Everything downstream of the stand-in is the package running for real.
const AC_PHP_CORE_TEST_PRELUDE: &str = r##"
(require 'cl-lib)
...
fn ac_php_core_oracle() -> CachedMelpaOracle {
    CachedMelpaOracle::new(AC_PHP_CORE_MELPA_PIN, "ac-php-core.el")
        .expect("prepare pinned ac-php-core source and dependencies below ./tmp")
        .with_prelude(AC_PHP_CORE_TEST_PRELUDE)
...
/// Multi-probe batch for `assert_ac_php_core_parity` cases (2a).
pub(crate) fn assert_ac_php_core_batch(cases: &[ParityBatchCase]) {
    let name = current_test_name();
    assert_oracle_batch_cases(ac_php_core_oracle(), &name, "ac_php_core_parity", cases);
}

// BEGIN generated package batch tests

#[test]
fn ac_php_core_package_batch() {
