//! Deep combo: match-string × replace-match × regexp-opt × looking-at ×
//! skip-chars-forward × skip-chars-backward × marker × overlay × textprop ×
//! undo × buffer-local × narrow.
//!
//! Stresses regex match/replace with buffer state: match-string,
//! replace-match, regexp-opt, looking-at, skip-chars, and skip-syntax.
//! These operations are tricky because they modify buffer content and
//! must correctly track markers, overlays, text properties, and undo state.
      ...
      (let ((m1 (copy-marker 10 nil))
        ...
        (while (re-search-forward "\\([a-z]+\\):\\([0-9]+\\)" nil t)
          (let ((name (match-string 1))
                (val (match-string 2)))
            (replace-match (format "%s=%s" (upcase name) val) t)))
