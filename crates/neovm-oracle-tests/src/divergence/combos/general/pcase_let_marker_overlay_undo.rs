//! Combo: pcase-let + markers + overlays + textprop + undo.
//! Tests pcase-let pattern matching with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_pcase_let_marker_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "plm")))
...
#[test]
fn combo_pcase_let_narrow_marker() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "pln")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
...
#[test]
fn combo_pcase_let_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "plc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'seg 'a)
      (put-text-property 6 10 'seg 'b)
      ...
      (let* ((ov (make-overlay 6 10))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "plc-clone")))
        (with-current-buffer clone
          (undo-boundary)
          (pcase-let ((`(,a ,b) (list "XX" "-")))
            (goto-char 6)
            (insert (concat a b)))
          (undo-boundary)
          (let ((mp (marker-position m))
...
#[test]
fn combo_pcase_let_multi_pattern() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "plp")))
    (with-current-buffer buf
...
#[test]
fn combo_pcase_let_overlay_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "plo")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
