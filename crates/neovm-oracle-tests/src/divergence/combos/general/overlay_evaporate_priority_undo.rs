//! Combo: multiple overlays same priority + evaporate + undo + markers.
//! Tests overlay priority resolution and evaporate behavior.
  ...
  (let ((buf (generate-new-buffer "ops")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 12))
             (ov2 (make-overlay 5 16))
             (ov3 (make-overlay 9 20))
        ...
        (let ((mp (marker-position m))
          ...
          (primitive-undo 1 buffer-undo-list)
          (widen)
          (list mp p1 p2 p3 os1 oe1 os2 oe2 bs
                (buffer-string)
                (marker-position m)))))
    (kill-buffer buf)))"#,
        expect,
