//! Complex combo batch 346 — `hash-table`/`obarray`/`sxhash` ultimate:
//! eq/eql/equal test variants, all weakness kinds with GC, maphash/remhash,
//! copy-hash-table independence, sxhash consistency, obarray intern/unintern.
...
(let ((ht (make-hash-table :test 'equal)))
  ...
  (let ((rec (record 'neo-cx346-mega :a :b :c)))
    (with-temp-buffer
      (buffer-enable-undo)
