//! Complex combo batch 238 — MEGA final: 5 extreme integration stress
//! tests covering 12+ subsystems for maximum divergence surface coverage.
...
(condition-case e
    (progn
      (require 'eieio)
      (defclass neo-cx238-obj ()
        ((value :initarg :value :initform 0)))
      (let ((lexical-binding t)
            (timer-fired nil)
            (calls nil)
            (env-val (let ((process-environment (cons "NEO_CX238=v1" process-environment)))
                       (string-trim (shell-command-to-string "echo $NEO_CX238"))))
            (exit-code (let ((p (make-process :name "neo-cx238-ec1"
                                                :command '("sh" "-c" "exit 3")))
        ...
        (sit-for 0.01)
        (defun neo-cx238-target (x) (push (list :primary x) calls) (* x 2))
        (advice-add 'neo-cx238-target :before
        ...
        (let* ((text "café 世界 😀 mega")
          ...
          (puthash "alpha" 1 ht)
          (puthash "beta" 2 ht)
          (let ((obj (make-instance 'neo-cx238-obj :value 0))
                (cl-loop-result
                 (cl-loop for k being the hash-keys of ht using (hash-values v)
                          collect (cons k v))))
            (let ((buf (get-buffer-create " *neo-cx238-mega-1*")))
              (with-current-buffer buf
                (buffer-enable-undo)
                (setq-local neo-cx238-buf :local)
                (insert text)
                ...
                (let ((m (set-marker (make-marker) 8))
                  ...
                  (let ((r (neo-cx238-target 5)))
                    (let ((state (list timer-fired env-val exit-code
                      ...
                      (undo)
                      (widen)
                      (advice-remove 'neo-cx238-target 'mega-advice)
                      (kill-buffer buf)
...
(let ((lexical-binding t)
      ...
      (calls nil))
  (defun neo-cx238-target () :orig)
  (advice-add 'neo-cx238-target :override (lambda () :overridden))
  (with-temp-buffer
    (buffer-enable-undo)
    ...
    (let ((m (set-marker (make-marker) 12))
      ...
      (let* ((matches (cl-loop for i from 0 below 3
             ...
             (advice-result (neo-cx238-target)))
        (let ((snapshot (list matches pcase-result time-str advice-result
          ...
          (widen)
          (jump-to-register ?c)
          (advice-remove 'neo-cx238-target (advice--p (advice-member-p nil 'neo-cx238-target)))
...
fn div_cx238_mega_3_process_buflocal_undo_textprop_overlay_coding_env_timer_weak_hash() {
    ...
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX238=v2" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX238"))))
      (exit-code (let ((p (make-process :name "neo-cx238-ec2"
                                          :command '("sh" "-c" "exit 6")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht)
        (garbage-collect)
  ...
  (run-with-timer 0 nil (lambda () (setq timer-fired :t)))
  (sit-for 0.01)
  (let ((buf (get-buffer-create " *neo-cx238-final*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx238-buf :local)
    ...
    (let ((p (make-process :name "neo-cx238-mega-3-p"
...
(let ((lexical-binding t)
      (calls nil))
  (defmacro neo-cx238-double (form) `(progn ,form ,form))
  (defun neo-cx238-target (x) (push (list :primary x) calls) (* x 2))
  (advice-add 'neo-cx238-target :before
              (lambda (x) (push (list :before x) calls))
              '((name . mega-advice)))
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-a") 'neo-cx238-cmd-a)
    (define-key map [f5] 'neo-cx238-f5)
    (letrec ((counter 0)
             (inc-fn (lambda () (cl-incf counter))))
      (let* ((ht (make-hash-table :test 'equal))
             (ob (make-obarray 31))
             (sym1 (intern "neo-cx238-alpha" ob))
             (text "café 世界"))
        (puthash "a" 1 ht)
        (puthash "b" 2 ht)
        (put sym1 'neo-cx238-prop :val)
        (let* ((hash (secure-hash 'sha256 text))
               (cl-loop-result (cl-loop for k being the hash-keys of ht using (hash-values v)
                                        collect (cons k v)))
               (macro-result (eval '(macroexpand '(neo-cx238-double (cl-incf counter))) t))
               (target-result (neo-cx238-target 5)))
          (with-temp-buffer
            (buffer-enable-undo)
...
fn div_cx238_mega_5_all_subsystem_final_ultimate_chaos_record_hash_coding_advice_macro() {
    ...
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX238=v3" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX238"))))
      (exit-code (let ((p (make-process :name "neo-cx238-final-ec"
                                          :command '("sh" "-c" "exit 7")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht)
        (garbage-collect)
        ...
        (process-exit-status p)))
      (ht (make-hash-table :test 'equal))
      (rec (record 'neo-cx238-tag :a :b :c))
      (lexical-binding t))
  (run-with-timer 0 nil (lambda () (setq timer-fired :t)))
  (sit-for 0.01)
  (puthash "alpha" 1 ht)
  ...
  (let* ((text "café 世界 final mega")
         (enc (encode-coding-string text 'utf-8))
         (hash (secure-hash 'sha256 enc))
         (buf (get-buffer-create " *neo-cx238-ultimate*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx238-local-counter 0)
      (insert text)
      (put-text-property 1 4 'face 'bold)
      (put-text-property 6 10 'display "XX")
      (let ((m (set-marker (make-marker) 8))
        ...
        (overlay-put ov 'evaporate t)
        (narrow-to-region 2 16)
        (cl-incf neo-cx238-local-counter)
        (aset rec 2 :modified)
