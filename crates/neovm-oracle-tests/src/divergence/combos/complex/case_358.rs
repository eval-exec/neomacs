//! Complex combo batch 358 — `display` property variants ultimate:
//! `(space :width N)`, `(space :align-to N)`, integer, string,
//! `(image ...)`, `(height N)`, `(raise N)`, `(left-fringe ...)`,
//! `(right-fringe ...)`, `(slice ...)`.
...
(with-temp-buffer
  ...
  (put-text-property 5 6 'display '(space :width 5 :height 2))
  (put-text-property 8 9 'display '(space :align-to 20))
...
(with-temp-buffer
  ...
  (add-to-invisibility-spec 'neo-cx358-h)
  (let ((ov (make-overlay 9 14)))
    (overlay-put ov 'invisible 'neo-cx358-h))
...
(with-temp-buffer
  (buffer-enable-undo)
  (insert "Display property ultimate mega test buffer content")
