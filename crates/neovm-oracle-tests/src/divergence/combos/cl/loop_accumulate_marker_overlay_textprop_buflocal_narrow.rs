//! Combo: cl-loop with collect/append/nconc + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests loop accumulation with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_loop_collect_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function cl-loop)""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "lcb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((lst (list 'a 'b 'c 'd))
               ...
               (result2 (cl-loop for x in lst append (list x x)))
               (result3 (cl-loop for x in lst nconc (list x))))
          (put-text-property (point-min) (point-max) 'z 'changed)
          (setf (char-after (point-min)) ?Z)
          (setf (marker-position m) 11)
          (goto-char (point-min))
          (insert (format "%s-%s-%s-%s-" result result2 result3 my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_loop_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "lco")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_loop_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function cl-loop)""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "lc1"))
        (b2 (generate-new-buffer "lc2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b2
        (undo-boundary)
        (let* ((lst (list 'd 'e 'f))
               (result (cl-loop for x in lst append (list x x))))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_loop_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_loop_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
