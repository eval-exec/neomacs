//! Oracle parity for GNU's just-in-time `syntax-propertize` inside
//! `parse-partial-sexp` (GNU `parse_sexp_propertize`, src/syntax.c:480): the
//! positions a scan enters, the `(start end)` pairs `syntax-propertize-function`
//! receives, `syntax-propertize--done` afterwards, the result and point.
...
#[test]
fn oracle_prop_parse_partial_sexp_propertizes_elisp_in_chunks() {
...
#[test]
fn oracle_prop_parse_partial_sexp_propertize_positions_sweep() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let form = r#"
...
/// had already read.
#[test]
fn oracle_prop_parse_partial_sexp_propertize_error_policy() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let form = r#"
(let (log out)
  (with-temp-buffer
