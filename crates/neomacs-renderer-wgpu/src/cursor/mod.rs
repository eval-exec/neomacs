...
pub struct CursorState {
    ...
    pub last_blink_toggle: neomacs_display_protocol::frame_time::EventTime,
...
impl CursorState {
    ...
    pub fn new(at: neomacs_display_protocol::frame_time::EventTime) -> Self {
...
impl CursorState {
    pub fn apply_visual_config(&mut self, config: &VisualConfig) {
    ...
    pub fn apply_config(&mut self, config: CursorConfigSnapshot) {
    ...
    pub fn animated_cursor(&self) -> Option<AnimatedCursor> {
    ...
    pub fn target_cloned(&self) -> Option<CursorTarget> {
        self.target.clone()
    }
    ...
    pub fn target_corners(target: &CursorTarget) -> [(f32, f32); 4] {
        match target.style {
    ...
    pub fn snap(&mut self, target: &CursorTarget) {
    ...
    pub fn reset_blink(&mut self, at: neomacs_display_protocol::frame_time::EventTime) {
        self.blink_on = true;
        self.last_blink_toggle = at;
    }

    pub fn force_blink_on(&mut self) -> bool {
        if self.blink_on {
            return false;
        }
        self.blink_on = true;
