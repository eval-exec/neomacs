...
//! | `NEOVM_JIT_COMPILE_STATS` | `=1`: print a running compile-stall summary line every 64 compiles (and every 50,000 dispatch consultations), plus a final `[neovm-jit-final*]` report when the command loop returns (`kill-emacs`, end of `--batch`, batch-error exit 255; not on death by signal). The final report separates the session from startup (`since_command_loop`). |
