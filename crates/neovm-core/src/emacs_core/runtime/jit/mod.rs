...
//! | `NEOVM_JIT_INLINE_HEAP_WRITE` | on | Inline stores at JIT `setcar`/`setcdr` sites when the cons lies outside the write barrier's owner window (`compile::heap_inline`); `=off` calls `neovm_jit_setcar`/`_setcdr` everywhere (single-build A/B). |
