...
//! | `NEOVM_JIT_AREF_SKIP_SLOT0` | off | MEASUREMENT ONLY (falsifier F-G (b), P3.2 F1a): inline `aref`/`aset` without the slot-0 tagged-vector test, and the `aset` shim without `classify_vector_slots`. Wrong on bool-vectors and legacy char-tables: valid only for runs whose hot paths create none (every bool-vector created while on is counted and the first warns). |
...
//! | `NEOVM_JIT_COLD_EXITS` | `=on` | Mark every exit block cold in both tiers (`compile::cold_exits`, P2.2 O0.2): precise-deopt and rerun blocks, the shared signal exit and per-site handler dispatches, the back-edge poll's slow path and the root-window grow calls, so Cranelift emits them after the hot code instead of between hot blocks. Read at compile time; unset/`off` marks nothing new, CLIF-identical (single-build A/B). Census: `cold_exits[deopt= rerun= signal= dispatch= poll= grow=]` in the `NEOVM_JIT_COMPILE_STATS=1` summary. Graduation: same-binary neutral or better on the six rows and org (P2.0 T0.3). |
...
//! | `NEOVM_REGEX_DFA` | `=on`, `=verify` | P3.3 existence DFA (`text/regex/dfa.rs`): `re_search` skips candidates the DFA rejects; `verify` also runs the matcher on every candidate and reports contradicted verdicts. `NEOVM_REGEX_DFA_STATS=1` prints the filter's counters (`[neovm-regex-dfa]`) at exit. Graduation: the F1′ gates (U3.1), then a default flip of its own (C8). |
//!
