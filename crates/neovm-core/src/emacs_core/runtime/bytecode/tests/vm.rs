...
fn vm_resolved_builtin_skips_redundant_string_writeback_classification() {
    ...
    // Leave the canonical name on the static-registry fallback, while the
    // alias and direct object exercise a function cell holding a subr.
    eval.obarray
        .set_symbol_function_id(intern("concat"), Value::NIL);
    for callee in [
...
#[test]
fn vm_builtin_writeback_cache_tracks_function_epoch() {
    crate::test_utils::init_test_tracing();
    let mut eval = Context::new_vm_runtime_harness();
    let concat = eval.eval_str("(symbol-function 'concat)").unwrap();
    let fillarray = eval.eval_str("(symbol-function 'fillarray)").unwrap();
    let alias = intern("vm-wb-changing-subr");
    for _ in 0..3 {
        for (callee, item, needs_writeback, expected) in [
            (concat, Value::string("!"), false, "abc"),
            (fillarray, Value::char('x'), true, "xxx"),
        ] {
            eval.obarray.set_symbol_function_id(alias, callee);
            let caller = string_call_retaining_argument(
                Value::from_sym_id(alias),
                Value::string("abc"),
                item,
            );
            // Check both a cache miss and the following hit, then change
            // the same cell to a builtin with the opposite writeback tag.
            for _ in 0..2 {
                reset_mutating_writeback_classification_count();
                let result = new_vm(&mut eval).execute(&caller, vec![]).unwrap();
                assert_eq!(result.as_utf8_str(), Some(expected));
                assert_eq!(
                    mutating_writeback_classification_count() > 0,
                    needs_writeback,
                    "writeback eligibility must follow the live function epoch"
                );
            }
        }
    }
