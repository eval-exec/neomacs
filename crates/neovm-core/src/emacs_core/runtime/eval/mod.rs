...
mod target_identity;
pub(crate) use target_identity::install_fixnum_limit_variables;
...
pub(crate) use subrs::{subr_call_entry_from_value, subr_entry_from_value};
...
impl GnuTimerTimestamp {
    pub(crate) fn now() -> Self {
        let (secs, usecs) = match neomacs_host_runtime::time::wall_time_since_unix_epoch() {
...
fn collect_thread_local_gc_roots(
    ...
    let scratch_t0 = neomacs_host_runtime::time::Instant::now();
...
pub struct Context {
    ...
    /// The evaluator's single filesystem namespace. It joins immutable
    /// product resources to the compile-selected or host-installed mutable
    /// backend without exposing those layers to Lisp primitives.
    pub(crate) editor_file_system: crate::emacs_core::fileio::EditorFileSystemNamespace,
...
pub(crate) fn plan_require_in_state(
    ...
    match super::load::resolve_load_path_file_with_filesystem(
        obarray,
        buf,
        &filename,
        requirement,
