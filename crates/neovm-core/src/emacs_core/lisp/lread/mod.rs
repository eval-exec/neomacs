...
mod locate;
pub(crate) use locate::{builtin_locate_file, builtin_locate_file_internal};
...
pub(crate) fn finish_read_char_exclusive_interactive_in_runtime(
    ...
    let deadline = read_plan
        .timeout()
        .map(|timeout| neomacs_host_runtime::time::Instant::now() + timeout);
    loop {
        let remaining = deadline.map(|deadline| {
            deadline.saturating_duration_since(neomacs_host_runtime::time::Instant::now())
        });
