...
fn ime_session_rejects_edits_after_point_or_buffer_changes() {
    ...
    eval.handle_ime_operation(ImeSessionId(1), ImeOperation::Replace {
        before_bytes: 4, after_bytes: 0, text: "wrong".into(),
    }).unwrap();
    assert_eq!(eval.eval_str("(buffer-string)").unwrap().as_utf8_str(), Some("safe"));
    eval.handle_ime_operation(ImeSessionId(2), ImeOperation::Begin).unwrap();
    eval.eval_str("(set-buffer (get-buffer-create \"other-ime\"))").unwrap();
    eval.handle_ime_operation(ImeSessionId(2), ImeOperation::Replace {
        before_bytes: 0, after_bytes: 0, text: "wrong".into(),
    }).unwrap();
    assert_eq!(eval.eval_str("(buffer-string)").unwrap().as_utf8_str(), Some(""));
}
...
fn ime_session_replaces_owned_unicode_text_and_rejects_stale_revisions() {
    ...
    eval.handle_ime_operation(ImeSessionId(1), ImeOperation::Replace {
        before_bytes: 7, after_bytes: 0, text: "好!".into(),
    }).unwrap();
    assert_eq!(eval.eval_str("(buffer-string)").unwrap().as_utf8_str(), Some("prefix:你好!"));
    eval.eval_str("(save-excursion (goto-char 1) (insert \"changed\"))").unwrap();
    eval.handle_ime_operation(ImeSessionId(1), ImeOperation::Replace {
        before_bytes: 4, after_bytes: 0, text: "wrong".into(),
    }).unwrap();
    assert_eq!(eval.eval_str("(buffer-string)").unwrap().as_utf8_str(), Some("changedprefix:你好!"));
}
...
fn ime_session_read_only_and_reentrant_hooks_cannot_redirect_replacement() {
    ...
    assert!(eval.handle_ime_operation(ImeSessionId(1), ImeOperation::Replace {
        before_bytes: 0, after_bytes: 0, text: "wrong".into(),
    }).is_err());
    ...
    assert_eq!(eval.eval_str("(buffer-string)").unwrap().as_utf8_str(), Some(""));
    assert_eq!(eval.eval_str("(progn (set-buffer \"*scratch*\") (buffer-string))").unwrap().as_utf8_str(), Some(""));
}
