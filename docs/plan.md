...
- Lock remaining raw `buf.read_only` mutators (`transpose-paragraphs`, `transpose-lines`) with mutation-aware checks.
...
- Add focused read-only corpora for `transpose-chars`, `transpose-paragraphs`, and `transpose-lines`.
